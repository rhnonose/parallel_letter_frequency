...
defmodule FrequencyTest do
  ...
  test "filter only letters, downcased" do
    assert Frequency.filter_letters("He's namé is Röbert Pâulsen.") |> Enum.join == "hesnaméisröbertpâulsen"
  end

  test "count a letter in a text" do
    pid = spawn(Frequency, :count_letter, [self])
    text = String.codepoints("He's namé is Röbert Pâulsen.")
    send pid, {:count, text, "e", self}
    receive do
      {:freq, 3, "e"} -> :ok
    after
      1000 -> flunk("Timeout")
